...
namespace FileService
{
    public class EmailManager
    {
        ...
        public EmailManager(bool critical)
        {
            _host = ConfigurationManager.AppSettings["htHost"];
            Int32.TryParse(ConfigurationManager.AppSettings["htPort"], out _port);
            _username = ConfigurationManager.AppSettings["htUsername"];
            _password = ConfigurationManager.AppSettings["htPassword"];
            _toDefault = ConfigurationManager.AppSettings["htEmailToEmergency"];
            _from = ConfigurationManager.AppSettings["htEmailFrom"];
        }

        public void Send(string[] recipientList, string emailSubject, string msg, bool bHtml, MailPriority mailPriority)
        {
            try
            {
                ...
                var mailMessage = new MailMessage { From = new MailAddress(_from), Subject = emailSubject, Body = msg, Priority = mailPriority };
                ...
                mailMessage.IsBodyHtml = bHtml; //remove
        ...
        public void Send(string[] recipientList, string emailSubject, string msg, bool bHtml)
        {
            try
            {
                if (string.IsNullOrEmpty(msg)) { return; }
