...
namespace InternalDataFeedService
{
    public class IQConnect
    {
        ...
        Process feedProcess = new Process();
        ...
        public void InitializeDataFeed()
        {
           ...
           */
           ...
           // txtProductVersion.Text = System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString();
            ...
            //feedProcess = System.Diagnostics.Process.Start("IQConnect.exe", "");


            feedProcess.StartInfo.FileName = "IQConnect.exe";

            try
            {
                feedProcess.Kill();
                feedProcess.WaitForExit();
            }
            catch (Exception ex)
            {
                //ignore
